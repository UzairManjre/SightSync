...
const PROMPTS: Record<string, { system: string; user: string }> = {
  ...
  text: {
    system:
      "You are an OCR assistant for a visually impaired person. " +
  ...
  face: {
    system:
      "You are a face description assistant for a visually impaired person. " +
...
export const analyzeImage = functions.https.onCall(
  ...
  async (request) => {
    const { imageBase64, featureType } = request.data as {
      imageBase64: string;
      featureType: string;
    };
    ...
    if (!featureType || !PROMPTS[featureType]) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        `featureType must be one of: ${Object.keys(PROMPTS).join(", ")}`
      );
    }
