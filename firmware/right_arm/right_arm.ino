...
class MyCommandCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (value.length() > 0) {
      Serial.print("Received Command on Right Arm: ");
...
void setup() {
  ...
  // Initialize BLE for RIGHT Arm
  BLEDevice::init("SightSync-R");
  ...
  pAdvertising->setMinPreferred(
      0x06); // functions that help with iPhone connection issues
  pAdvertising->setMinPreferred(0x12);
  BLEDevice::startAdvertising();
  Serial.println(
      "RIGHT ARM BLE Advertising Started - Waiting for connections...");
}
...
void loop() {
  ...
  if (deviceConnected && !oldDeviceConnected) {
    oldDeviceConnected = deviceConnected;
    Serial.println("Device Connected to Right Arm!");
  }
  delay(100);
}
